...
export class AddComponent {
  ...
  private _loadFormGroup(): void {
    ...
    if (this.ediData) {
      ...
      this.cobradorForm.controls["codCobrador"].setValue(
        this.ediData.codCobrador
      );
    ...
    this._cobradorApiService.createCobrador(cobrador).subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this._sonotifyService.info("El registro se guardo sin problema", {
            position: SnotifyPosition.rightTop,
          });
          this._dialogRef.close("save");
        } else {
          this._sonotifyService.error(response.errors[0], {
            position: SnotifyPosition.rightTop,
          });
        }
      },
  ...
  private _edit(cobrador: IRequestCreateCobrador) {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    this._cobradorApiService
      .update(this.ediData.id as number, cobrador)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.cobradorForm.reset();
            this._sonotifyService.info(
              "El registro se actualizo sin problema",
              { position: SnotifyPosition.rightTop }
            );
