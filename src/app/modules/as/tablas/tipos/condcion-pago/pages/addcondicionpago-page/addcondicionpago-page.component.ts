...
export class AddcondicionpagoPageComponent {
  ...
  private _edit(datos: ICondicionPagoEdit) {
    this._condicionPagoApiService
      .updateCondicionPago(this.editData.id, datos)
      .subscribe({
        next: (response) => {
          console.log(response);
          if (response.success) {
            this.codicionPagoForm.reset();
            this._snotifyService.info("El Registro de Actualizo sin problema", {
              position: SnotifyPosition.rightTop,
            });
  ...
  private _loadFormGroup(): void {
    ...
    if (this.editData) {
      ...
      this.codicionPagoForm.controls["codCondicionPago"].setValue(
        this.editData.codCondicionPago
      );
      this.codicionPagoForm.controls["codCondicionPago"].disable();
      this.codicionPagoForm.controls["descripcion"].setValue(
        this.editData.descripcion
      );
      this.codicionPagoForm.controls["diasNeto"].setValue(
        this.editData.diasNeto
      );
      this.codicionPagoForm.controls["descuentoContado"].setValue(
        this.editData.descuentoContado
      );
      this.codicionPagoForm.controls["permitePlanPagos"].setValue(
        this.editData.pagosParciales
      );
      this.codicionPagoForm.controls["plazo"].setValue(
        this.editData.plazoCondPago
      );
    }
  }
