...
export class AddCatProveedorPageComponent {
  ...
  private _loadFormulario(): void {
    ...
    if (this.editData) {
      ...
      this.formCategoriaProveedor.controls["categoria"].setValue(
        this.editData.codCategoriaProveedor
      );
      this.formCategoriaProveedor.controls["categoria"].disable();
      this.formCategoriaProveedor.controls["nombre"].setValue(
        this.editData.descripcion
      );
      //Tab Cuentas por Pagar
      this.formCategoriaProveedor.controls["cuentaPagar"].setValue(
        this.editData.ctrCtaCPId
      );
      this.formCategoriaProveedor.controls["ajustexRedondeo"].setValue(
        this.editData.ctrCtaAjusteRedondeoId
      );
      this.formCategoriaProveedor.controls["letraxPagar"].setValue(
        this.editData.ctrCtaLPId
      );
      this.formCategoriaProveedor.controls["prontoPago"].setValue(
        this.editData.ctrCtaProntoPagoCPId
      );
      this.formCategoriaProveedor.controls["comisiones"].setValue(
        this.editData.ctrCtaComisionesCPId
      );
      this.formCategoriaProveedor.controls["anticipos"].setValue(
        this.editData.ctrCtaAnticipoCPId
      );
      this.formCategoriaProveedor.controls["cierreDebito"].setValue(
        this.editData.ctrCtrDebitoCPId
      );
      this.formCategoriaProveedor.controls["cierreCredito"].setValue(
        this.editData.ctrCtaCreditoCPId
      );
      //Tab Impuesto y Rubros
      this.formCategoriaProveedor.controls["impuesto1"].setValue(
        this.editData.ctrCtaImpuesto1CPId
      );
      this.formCategoriaProveedor.controls["impuesto2"].setValue(
        this.editData.ctrCtaImpuesto2CPId
      );
      this.formCategoriaProveedor.controls["rubro1"].setValue(
        this.editData.ctrCtaRubro1CPId
      );
      this.formCategoriaProveedor.controls["rubro2"].setValue(
  ...
  private _edit(data: IRequestCreateCtaProveedor): void {
    console.log(data);
    this._catProveedorApiService
      .updateCategoriaProveedor(this.editData.id as number, data)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.formCategoriaProveedor.reset();
