...
import {
  IResponseNivelPrecio,
  IRequestCreateNivelPrecio,
} from "./../../services/nivel-precio-api-model-interface";
import {
  MAT_DIALOG_DATA,
  MatDialog,
  MatDialogRef,
...
export class AddNivelPrecioPageComponent implements OnInit {
  ...
  private _loadFormulario(): void {
    ...
    if (this.ediData) {
      ...
      this.formNivelPrecio.controls["nivelPrecio"].setValue(
        this.ediData.codNivelPrecio
      );
      this.formNivelPrecio.controls["nivelPrecio"].disable();
      this.formNivelPrecio.controls["moneda"].setValue(this.ediData.moneda);
      this.formNivelPrecio.controls["esquema"].setValue(
        this.ediData.esquemaTrabajo
      );
      if (this.ediData.condicionPagoId) {
        this.formNivelPrecio.controls["condPago"].setValue(
          this.ediData.condicionPagoId
        );
      }
    }
  }
  ...
  private _update(datos: IRequestCreateNivelPrecio) {
    this._nivelPrecioApiService
      .updateNivelprecio(this.ediData.id, datos)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.formNivelPrecio.reset();
            this._snotifyService.info("El registro se actualizo con existo", {
              position: SnotifyPosition.rightTop,
            });
