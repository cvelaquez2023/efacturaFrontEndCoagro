...
import {
  ICreateCategoriaArticulo,
  IResponseCategoriaArticulo,
} from "./categoria-articulo-api-model-interface";
...
export class CategoriaArticuloApiService {
  ...
  getCatArticulo(
    page?: number,
    rows?: number
  ): Observable<IResponse<IResponseCategoriaArticulo[]>> {
  ...
  createCatArticulo(
    catArticulo: ICreateCategoriaArticulo
  ...
  updateCategoria(
    idCategoaria: number,
    categoria: ICreateCategoriaArticulo
  ): Observable<IResponse<number>> {
    // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
