...
} from "./bodega-api-model-interface";
...
export class BodegaApiService {
  ...
  getBodega(
    page?: number,
    rows?: number
  ): Observable<IResponse<IResponseBodega[]>> {
    let url = URL_BODEGA;
  ...
  update(
    idBodega: number,
    bodega: IResponseCreateBodega
  ): Observable<IResponse<number>> {
    const url = URL_BODEGA + "/" + idBodega;
    return this._httpClient.put<IResponse<number>>(url, bodega);
  }
  getBodegaLocalizacion(
    idBodega: number
  ): Observable<IResponse<IResponseLocaizacionBodega[]>> {
    // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
