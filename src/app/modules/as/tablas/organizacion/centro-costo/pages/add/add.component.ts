...
export class AddComponent {
  ...
  private _loadFormGroup(): void {
    ...
    if (this.ediData) {
      ...
      this.centrocostoForm.controls["codCentroCosto"].setValue(
        this.ediData.codCentroCosto
      );
      this.centrocostoForm.controls["codCentroCosto"].disable();
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      this.centrocostoForm.controls["descripcion"].setValue(
        this.ediData.descripcion
      );
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      this.centrocostoForm.controls["aceptaDatos"].setValue(
        this.ediData.aceptaDatos
      );
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
  ...
  private _edit(centrocosto: IRequestCentroCosto) {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    this._centroCostoApiService
      .updateCentroCosto(this.ediData.id as number, centrocosto)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.centrocostoForm.reset();
