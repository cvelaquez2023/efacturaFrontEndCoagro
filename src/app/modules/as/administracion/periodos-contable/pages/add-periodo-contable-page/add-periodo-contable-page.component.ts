...
export class AddPeriodoContablePageComponent {
  ...
  private _loadFormGroup(): void {
    ...
    if (this.editData) {
      ...
      this.PeriodosForm.controls["descripcion"].setValue(
        this.editData.descripcion
      );
      this.PeriodosForm.controls["contabilidad"].setValue(
        this.editData.contabilidad
      );
    }
  }
  clickSave(): void {
    ...
    this._peridosContApiService.createPeriodoContable(periodo).subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this._snotifyService.info("El resgisto se guardo sin problema", {
  ...
  private _edit(periodo: IResponsePeriodosContables) {
    ...
    this._peridosContApiService
      .updatePeriodoContable(this.editData.id as number, periodo)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.PeriodosForm.reset();
