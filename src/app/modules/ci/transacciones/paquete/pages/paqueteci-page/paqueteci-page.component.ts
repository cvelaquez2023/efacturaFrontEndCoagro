...
import {
  IConsultaPaqueteInvApiModel,
  ICreateMovInventario,
} from "./../../model/paqueteInv-api-model-interface";
...
export class PaqueteciPageComponent implements OnInit, AfterViewInit {
  ...
  private _loadDocumento(idPaquete: number): void {
    this._paqueteciApiService.getDocumnetoInvEnc().subscribe({
      next: (response) => {
        ...
        const _doccumentoInvEncNew = documentoinvEncNew.filter(
          (f) => f.paqueteInventarioId == idPaquete
        );
  ...
  clickAplicar(element: IConsultaDocumnetoInvEnca): void {
    ...
    if (element.aprobado) {
      ...
      this._snotifyService.error("El Paquete Tiene que estar Aprobado", {
