...
} from "./../../../consecutivos/model/IResponseConsecutivoCi";
...
export class AddTransconfigurablePageComponent implements OnInit {
  ...
  private _loadajustes(id: string): void {
    ...
    this._consecutivoApiService.getConsInvAjuste().subscribe({
      next: (response) => {
        ...
        this._ajuste = consAjusteNew.filter(
          (p) => p.ajusteConfig.ajusteBase === id
        );
        ...
        for (let i = 0; i < this._ajuste.length; i++) {
          ...
          const datos = this._consecutivos.filter(
            (p) => p.id == element.consecutivoInvId
          );
  ...
  public selChange(e: MatSelectionListChange): void {
    ...
    const resultado: SubtipoInterface | undefined = this._subtipo?.find(
      (t) => t.codigo == selecion
    );
  ...
  public selChange2(e: MatSelectionListChange): void {
    ...
    const resultado2: SubtipoInterface | undefined = this._subsubtipo?.find(
      (t) => t.codigo == selecion2
    );
