...
export class AddConsecutivoPageComponent implements OnInit, AfterViewInit {
  ...
  private _loadFormGroup(): void {
    ...
    if (this.editData) {
      ...
      this.consecutivoForm.controls["codConsecutivo"].setValue(
        this.editData.codConsecutivo
      );
      this.consecutivoForm.controls["codConsecutivo"].disable();
      this.consecutivoForm.controls["descripcion"].setValue(
      ...
      this.consecutivoForm.controls["siguienteValor"].setValue(
        this.editData.siguienteConsec
      );
      this.consecutivoForm.controls["consecutivoPara"].setValue(
        this.editData.tipo
      );
      this.consecutivoForm.controls["consecutivoPara"].disable();
      this.consecutivoForm.controls["editable"].setValue(
        this.editData.editable
      );
      this.consecutivoForm.controls["asociado"].setValue(
        this.editData.todasTrans
      );
    }
  }
  private _loadajustes(): void {
    this._consecutivoApiService.getConsInvAjuste().subscribe({
      next: (response) => {
        ...
        const ajuste = consAjusteNew.filter(
          (p) => p.consecutivoInvId === this.editData.id
        );
      ...
      buttons: [
        {
          ...
          action: (toast) => {
            ...
            this._consecutivoApiService
              .deleteConstInvAjuste(id)
              .subscribe((response) => {
  ...
  adicionar(): void {
    ...
    this._consecutivoApiService
      .createConsInvAjuste(sendConsInvAjConf)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this._snotifyService.info("El registro de adiciono con existo", {
              position: SnotifyPosition.rightTop,
            });
            this._loadajustes();
          } else {
