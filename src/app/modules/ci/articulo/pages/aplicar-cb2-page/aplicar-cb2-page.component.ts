...
export class AplicarCb2PageComponent {
  ...
  private _loadDatos(): void {
    ...
    this._service.getDteCp(this.editData.Dte_Id as number).subscribe({
      next: (response) => {
        ...
        //datalle 1
        this.formAplicarCh.controls["fechadoc"].setValue(
        ...
        this.formAplicarCh.controls["impuesto"].setValue(
        ...
        this.formAplicarCh.controls["proveedor"].setValue(
          this.datoCp[0].nombre
        );

        //this.formAplicarCh.controls['montoprovision'].setValue(this.datoCp[0].monto);
        this.formAplicarCh.controls["subtotal"].setValue(
        ...
        this.formAplicarCh.controls["detalle"].setValue(
  ...
  updateVale(vale: any): void {
    this.formAplicarCh
      .get("departamento")
      ?.setValue(
        this.listaVale.find((r) => r.CONSECUTIVO == vale)?.DEPARTAMENTO || ""
      );
    this.formAplicarCh
      .get("concepto")
      ?.setValue(
        this.listaVale.find((r) => r.CONSECUTIVO == vale)?.CONCEPTO_VALE || ""
      );
    this.formAplicarCh
      .get("beneficionario")
      ?.setValue(
        this.listaVale.find((r) => r.CONSECUTIVO == vale)?.BENEFICIARIO || ""
      );
    ...
    this.formAplicarCh
      .get("montoprovision")
      ?.setValue(
        this.listaVale.find((r) => r.CONSECUTIVO == vale)?.MONTO_CAJA || 0
      );
    this.formAplicarCh
      .get("montoliquidacion")
      ?.setValue(
        this.listaVale.find((r) => r.CONSECUTIVO == vale)?.MONTO_VALE || 0
      );

    const disponible =
      (this.montoprovisionField.value as number) -
      (this.montoliquidacionField.value as number);
    if (disponible < this.total) {
      this._snotifyService.error(
        "El Monto de documento es mayor a lo Disponible",
        {
  ...
  private _lodadBeneficiario(): void {
    console.log("entramos a beneficioario");
  ...
  clickSave(): void {
    ...
    const conse =
      this.listaVale.find((r) => r.CONSECUTIVO == vale)?.CONSECUTIVO || 0;
    ...
    const docsSoporte = {
      ...
      _fecha: this.convertiFecha(this.fechaDocField.value as string),
      _fecha_rigue: this.convertiFecha(this.fechaDocField.value as string),
  ...
  get fechaDocField(): AbstractControl {
    return this.formAplicarCh.get("fechadoc")!;
  }
