...
export class AddArticuloBodegaPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(() => {
      ...
      if (this.editData.activo) {
        ...
        this._snotifyService.info("El Articulo Tiene que estar Activo", {
  ...
  private _save(existencia: IResponseCreateExistenciaBodega) {
    this._existenciaBodegaApiService
      .crearExistenciaBodega(existencia)
      .subscribe({
        next: (response) => {
          if (response.success) {
  ...
  private _update(existencia: IResponseCreateExistenciaBodega) {
    this._existenciaBodegaApiService
      .updateExistenciaBodega(this.existenciaBodegaId, existencia)
      .subscribe({
        next: (response) => {
          this.formGroup.reset();
          this._snotifyService.info("El registro Actualizdo ", {
            position: SnotifyPosition.rightTop,
          });
          this._loadExistenciaBodega(1, 10000);
        },
  ...
  private _loadExistenciaBodega(page: number, rows: number): void {
    this._existenciaBodegaApiService.getExistenciaBodega(page, rows).subscribe({
      next: (response) => {
        const resultado = response.result.filter(
          (e) => e.articuloId == this.editData.id
        );
