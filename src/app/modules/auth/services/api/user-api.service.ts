...
const URL_SEND_TOKEN_RESET_PASSWORD =
  environment.host + "/ERPADMIN/Usuario/SendTokenToResetPassword";
const URL_RESET_PASSWORD = environment.host + "/ERPADMIN/Usuario/ResetPassword";
const URL_CHANGE_PASSWORD =
  environment.host + "/ERPMADIN/Usuario/ChangePassword";
...
export class UserApiService {
  ...
  getListarUsuario(): Observable<IResponse<IResponseListarUsuario[]>> {
    return this._httpClient.get<IResponse<IResponseListarUsuario[]>>(
      URL_USER_LIST
    );
  }
  ...
  conjunto(
    email?: string,
    conjuntoId?: string
  ): Observable<IResponse<IResponseConjunto[]>> {
  ...
  sendTokenToResestPassword(email: string): Observable<IResponse<string>> {
    return this._httpClient.post<IResponse<string>>(
