...
import { IResponseBodegaAsocRt, IResponseBodegaFr } from '@app/modules/fr/administracion/bodegas/model/bodega-fr-model-interface';
...
export class CargaBodegasPageComponent implements OnInit, OnDestroy {
		...
		return this._bodegaFrApiService.getBodegas(1, 100).pipe(
			expand((response) =>
				response.success && response.pagination.page < response.pagination.totalPages
					? this._bodegaFrApiService.getBodegas(response.pagination.page + 1, 100)
					: EMPTY
			),
			map((response) => {
				if (response.success) {
	...
	private _cargarPagina(): void {
		this.cargando = true;
		this._bodegaErpApiService.getBodegasErp(this.q, this.page, this.limit).subscribe({
			next: (response) => {
				this.cargando = false;
				if (response.success) {
					this.bodegasErp = response.result.filter((b) => !this._bodegasCargadas.has(b.BODEGA));
					this.total = response.pagination ? response.pagination.total : response.result.length;
				} else {
					this._snotifyService.error(response.errors?.[0] ?? 'Error al consultar las bodegas del ERP', {
			...
			error: () => {
				this.cargando = false;
				this._snotifyService.error('No fue posible consultar las bodegas del ERP', { position: SnotifyPosition.rightTop });
			}
		});
	}
	...
	toggleBodega(element: IResponseBodegaErp, checked: boolean): void {
		if (checked) {
			this.selectedBodegas.set(element.BODEGA, { BODEGA: element.BODEGA, NOMBRE: element.NOMBRE });
		} else {
	...
	cargarSeleccionados(): void {
		...
		const rtError = (): Observable<IResponse<IResponseBodegaFr>> =>
			of<IResponse<IResponseBodegaFr>>({ success: false, errors: ['Error de conexión'], result: null as unknown as IResponseBodegaFr });
		...
		const yaRegistrada = (errors: string[]): boolean => (errors?.[0] ?? '').toLowerCase().includes('ya esta registrada');
		...
		forkJoin(peticiones).subscribe((respuestas) => {
			...
			if (exitosos) {
				this._snotifyService.info(`${exitosos} bodega(s) cargada(s) correctamente`, { position: SnotifyPosition.rightTop });
			}
			if (fallidos) {
				this._snotifyService.warning(`${fallidos} bodega(s) no se pudieron cargar: ${primerError}`, {
					position: SnotifyPosition.rightTop
				});
			}
