...
export class CargaArticulosPageComponent implements OnInit, OnDestroy {
    ...
    this._grupoArticuloApiService
      ...
      .subscribe((asociaciones) => {
        asociaciones.forEach((a) =>
          this._addGrupoArticulo(a.ARTICULO, a.GRUPO_ARTICULO)
        );
      });
  }
      ...
      map((response) => {
        if (response.success) {
          response.result.forEach((a) =>
            this._articulosCargados.add(a.ARTICULO)
          );
        }
      }),
      last()
  ...
  private _cargarPagina(): void {
    this.cargando = true;
    this._articuloErpApiService
      .getArticulosErp(this.q, this.page, this.limit)
      .subscribe({
        next: (response) => {
          this.cargando = false;
          if (response.success) {
            ...
            this._snotifyService.error(
              response.errors?.[0] ??
                "Error al consultar los articulos del ERP",
              {
        ...
        error: () => {
          this.cargando = false;
          this._snotifyService.error(
            "No fue posible consultar los articulos del ERP",
            { position: SnotifyPosition.rightTop }
          );
        },
      });
  ...
  toggleArticulo(element: IResponseArticuloErp, checked: boolean): void {
    if (checked) {
      this.selectedArticulos.set(element.ARTICULO, {
  ...
  cargarSeleccionados(): void {
    ...
    const peticiones = seleccionados.map((articulo) => {
      ...
      const guardarArticulo$: Observable<IResponse<IResponseArticuloFr>> =
        yaCargado
                ...
                map(
                  (response) =>
                    ({
                      ...
                      result: articulo as unknown as IResponseArticuloFr,
                    } as IResponse<IResponseArticuloFr>)
                )
              )
      ...
      return guardarArticulo$.pipe(
        switchMap((response) => {
          ...
          if (
            this._gruposPorArticulo
              .get(articulo.ARTICULO)
              ?.has(this.grupoSeleccionado)
          ) {
          ...
          return this._grupoArticuloApiService
            .asociarArticulo({
              grupoArticulo: this.grupoSeleccionado,
              articulo: articulo.ARTICULO,
            })
            .pipe(
              map((asocResponse) => {
                if (asocResponse.success) {
                  this._addGrupoArticulo(
                    articulo.ARTICULO,
        ...
        catchError(() =>
          of<IResponse<IResponseArticuloFr>>({
            success: false,
            errors: ["Error de conexión"],
            result: null as unknown as IResponseArticuloFr,
          })
        )
      );
    });
    ...
    forkJoin(peticiones).subscribe((respuestas) => {
      ...
      if (exitosos) {
        this._snotifyService.info(
          `${exitosos} articulo(s) cargado(s) correctamente`,
          { position: SnotifyPosition.rightTop }
        );
      }
      if (fallidos) {
        this._snotifyService.warning(
          `${fallidos} articulo(s) no se pudieron cargar: ${primerError}`,
          {
            position: SnotifyPosition.rightTop,
          }
        );
      ...
      if (gruposFallidos) {
        this._snotifyService.warning(
          `${gruposFallidos} articulo(s) no se pudieron vincular al grupo de artículo seleccionado`,
          {
            position: SnotifyPosition.rightTop,
          }
        );
      ...
      if (articulosYaEnGrupo.length) {
        const mensaje =
          articulosYaEnGrupo.length === 1
            ...
            : `Los artículos ${articulosYaEnGrupo.join(
                ", "
              )} ya pertenecen al grupo ${this.grupoSeleccionado}`;
        this._snotifyService.warning(mensaje, {
          position: SnotifyPosition.rightTop,
        });
      }
