...
import { IResponseClienteAsocRt, IResponseClienteFr } from '@app/modules/fr/administracion/clientes/model/cliente-fr-model-interface';
...
export class CargaClientesPageComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		this._consulta$
			.pipe(
				switchMap(() => {
					this.cargando = true;
					return this._clienteErpApiService.getClientesErp(this.q, this.page, this.limit, 'S').pipe(
						catchError(() =>
							of<IResponsePaginadaClienteErp<IResponseClienteErp[]>>({
								success: false,
								errors: ['No fue posible consultar los clientes del ERP'],
								result: [],
								pagination: { page: 1, limit: this.limit, total: 0, totalPages: 1 }
							})
						)
					);
				})
			...
			.subscribe((response) => {
				this.cargando = false;
				if (response.success) {
					this.clientesErp = response.result.filter((c) => !this._clientesCargados.has(c.CLIENTE));
					this.total = response.pagination ? response.pagination.total : response.result.length;
				} else {
					this._snotifyService.error(response.errors?.[0] ?? 'Error al consultar los clientes del ERP', {
		...
		return this._clienteFrApiService.getClientes(1, 100).pipe(
			expand((response) =>
				response.success && response.pagination.page < response.pagination.totalPages
					? this._clienteFrApiService.getClientes(response.pagination.page + 1, 100)
					: EMPTY
			),
			map((response) => {
				if (response.success) {
	...
	toggleCliente(element: IResponseClienteErp, checked: boolean): void {
		if (checked) {
			this.selectedClientes.set(element.CLIENTE, { CLIENTE: element.CLIENTE, NOMBRE: element.NOMBRE });
		} else {
	...
	cargarSeleccionados(): void {
		...
		const rtError = (): Observable<IResponse<IResponseClienteFr>> =>
			of<IResponse<IResponseClienteFr>>({ success: false, errors: ['Error de conexión'], result: null as unknown as IResponseClienteFr });
		...
		const yaRegistrado = (errors: string[]): boolean => (errors?.[0] ?? '').toLowerCase().includes('ya esta registrado');
		...
		const peticiones = seleccionados.map((cliente) =>
			this._clienteFrApiService.createCliente({ cliente: cliente.CLIENTE, nombre: cliente.NOMBRE }).pipe(
				catchError(rtError),
				switchMap((rtResponse) => {
					...
					return this._clienteAsocRtApiService
						.createCliente({ cliente: cliente.CLIENTE, codigo: cliente.CLIENTE })
		...
		forkJoin(peticiones).subscribe((respuestas) => {
			...
			if (exitosos) {
				this._snotifyService.info(`${exitosos} cliente(s) cargado(s) correctamente`, { position: SnotifyPosition.rightTop });
			}
			if (fallidos) {
				this._snotifyService.warning(`${fallidos} cliente(s) no se pudieron cargar: ${primerError}`, {
					position: SnotifyPosition.rightTop
				});
			}
