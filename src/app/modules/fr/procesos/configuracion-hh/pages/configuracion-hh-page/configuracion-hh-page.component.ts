...
export class ConfiguracionHhPageComponent {
	...
	quitar(item: IHandheldItem): void {
		this.seleccionados = this.seleccionados.filter((h) => h.codigo !== item.codigo);
		this.disponibles = [...this.disponibles, item];
	}

	procesar(): void {
		if (!this.origen || !this.seleccionados.length) {
			this._snotifyService.warning('Debe elegir un HandHeld origen y al menos un destino', { position: SnotifyPosition.rightTop });
			return;
		}
		this._snotifyService.info('La copia de configuracion finalizo correctamente', { position: SnotifyPosition.rightTop });
	}
}
