...
import { ICreateGrupoArticuloModel, IResponseArticuloRt, IResponseGrupoArticulo } from '../../model/grupo-articulo-fr-model-interface';
...
export class AddGrupoArticuloComponent implements AfterViewInit {
	...
	private _loadFormGroup(): void {
		...
		if (this.ediData) {
			...
			this.grupoForm.controls['grupoArticulo'].setValue(this.ediData.GRUPO_ARTICULO);
	...
	private _loadArticulosAsociados(): void {
		...
		forkJoin({
			...
			vinculados: this._grupoArticuloApiService.getAllArticulosAsociados(this.ediData.GRUPO_ARTICULO)
		}).subscribe({
			...
			error: () => {
				this._snotifyService.error('No fue posible consultar los artículos asociados al grupo', { position: SnotifyPosition.rightTop });
				this.cargandoArticulos = false;
			}
		});
	}
	...
	private _save(grupoArticulo: ICreateGrupoArticuloModel): void {
		this._grupoArticuloApiService.createGrupoArticulo(grupoArticulo).subscribe({
			...
			error: () => {
				this.guardando = false;
				this._snotifyService.error('Ocurrió un error al guardar el grupo de artículo', { position: SnotifyPosition.rightTop });
			}
		});
	}

	private _edit(grupoArticulo: ICreateGrupoArticuloModel): void {
		this._grupoArticuloApiService.updateGrupoArticulo(this.ediData.GRUPO_ARTICULO, grupoArticulo).subscribe({
			next: (response) => {
				this.guardando = false;
				if (response.success) {
					this.grupoForm.reset();
			...
			error: () => {
				this.guardando = false;
				this._snotifyService.error('Ocurrió un error al actualizar el grupo de artículo', { position: SnotifyPosition.rightTop });
			}
		});
	}
