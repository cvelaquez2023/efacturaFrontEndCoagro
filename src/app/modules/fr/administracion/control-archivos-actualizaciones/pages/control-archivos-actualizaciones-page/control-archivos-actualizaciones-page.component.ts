...
const MOCK_ACTUALIZACIONES: IActualizacionFr[] = [
	{ nombre: 'ReporteFacturaSV_v3', tipo: 'Reporte', fecha: '2026-07-20', handhelds: 'HH01, HH02, HH03', publicada: true },
	{ nombre: 'FR_Mobile_Setup_7.01', tipo: 'Paquete', fecha: '2026-08-01', handhelds: 'HH01, HH02', publicada: false }
];
...
export class ControlArchivosActualizacionesPageComponent implements OnInit, AfterViewInit {
	constructor(
	...
	displayedColumns: string[] = ['nombre', 'tipo', 'fecha', 'handhelds', 'publicada', 'actions'];
		...
		this._dialog
			...
			.subscribe((result: IActualizacionFr) => {
				if (result) {
					...
					this.listActualizaciones.data = this._data;
					this._snotifyService.info('La actualizacion se creo sin problema', { position: SnotifyPosition.rightTop });
				}
			});
	}

	publicar(element: IActualizacionFr): void {
		this._data = this._data.map((item) => (item.nombre === element.nombre ? { ...item, publicada: true } : item));
		this.listActualizaciones.data = this._data;
		this._snotifyService.info('La actualizacion ha sido publicada', { position: SnotifyPosition.rightTop });
	}
}
