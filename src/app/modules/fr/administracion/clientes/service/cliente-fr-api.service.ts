...
export class ClienteFrApiService {
  ...
  getClientes(
    page?: number,
    limit?: number
  ): Observable<IResponsePaginada<IResponseClienteFr[]>> {
    let params = new HttpParams();
    if (page != null) params = params.set("page", page);
    if (limit != null) params = params.set("limit", limit);
    return this._httpClient.get<IResponsePaginada<IResponseClienteFr[]>>(
      URL_CLIENTE,
      { params }
    );
  }
  ...
  getAllClientes(): Observable<IResponseClienteFr[]> {
    ...
    return pagina(1).pipe(
      ...
      reduce(
        (acumulado: IResponseClienteFr[], response) =>
          acumulado.concat(response.result || []),
        [] as IResponseClienteFr[]
      )
    );
  }
  createCliente(
    cliente: ICreateClienteFrModel
  ): Observable<IResponse<IResponseClienteFr>> {
    return this._httpClient.post<IResponse<IResponseClienteFr>>(
      URL_CLIENTE,
      cliente
    );
  }
