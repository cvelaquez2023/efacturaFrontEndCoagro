...
export class ClienteAsocRtApiService {
	constructor(private _httpClient: HttpClient, private _clienteFrApiService: ClienteFrApiService) {}

	getClientes(page?: number, limit?: number): Observable<IResponsePaginada<IResponseClienteAsocRt[]>> {
		let params = new HttpParams();
		if (page != null) params = params.set('page', page);
		if (limit != null) params = params.set('limit', limit);
		return this._httpClient.get<IResponsePaginada<IResponseClienteAsocRt[]>>(URL_CLIENTE_ASOC, { params });
	}
	...
	private _getAllClientes(): Observable<IResponseClienteAsocRt[]> {
		...
		return pagina(1).pipe(
			...
			reduce((acumulado: IResponseClienteAsocRt[], response) => acumulado.concat(response.result || []), [] as IResponseClienteAsocRt[])
		);
	}
	...
	getClientesConNombre(): Observable<IResponse<IResponseClienteConAsoc[]>> {
		return forkJoin([this._getAllClientes(), this._clienteFrApiService.getAllClientes()]).pipe(
			map(([asoc, rt]) => {
				...
				const result: IResponseClienteConAsoc[] = asoc.map((a) => {
					...
					return {
						...
						FECHA_ACTUALIZACION_UBICACION: info?.FECHA_ACTUALIZACION_UBICACION ?? null,
						CODIGO: a.CODIGO,
	...
	createCliente(cliente: ICreateClienteAsocRtModel): Observable<IResponse<IResponseClienteAsocRt>> {
		return this._httpClient.post<IResponse<IResponseClienteAsocRt>>(URL_CLIENTE_ASOC, cliente);
	}
