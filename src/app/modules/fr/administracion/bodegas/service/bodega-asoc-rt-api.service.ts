...
export class BodegaAsocRtApiService {
	constructor(private _httpClient: HttpClient, private _bodegaFrApiService: BodegaFrApiService) {}

	getBodegas(): Observable<IResponse<IResponseBodegaAsocRt[]>> {
		return this._httpClient.get<IResponse<IResponseBodegaAsocRt[]>>(URL_BODEGA_ASOC);
	}
	...
	getBodegasConNombre(): Observable<IResponse<IResponseBodegaConAsoc[]>> {
		return forkJoin([this.getBodegas(), this._bodegaFrApiService.getBodegas()]).pipe(
			map(([asoc, rt]) => {
				if (!asoc.success) {
					return { result: [], success: false, errors: asoc.errors };
				}
				const porBodega = new Map((rt.success ? rt.result : []).map((b) => [b.BODEGA, b]));
	...
	createBodega(bodega: ICreateBodegaAsocRtModel): Observable<IResponse<IResponseBodegaAsocRt>> {
		return this._httpClient.post<IResponse<IResponseBodegaAsocRt>>(URL_BODEGA_ASOC, bodega);
	}
