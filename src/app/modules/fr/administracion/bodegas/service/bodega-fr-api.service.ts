...
export class BodegaFrApiService {
  ...
  getBodegas(
    page?: number,
    limit?: number
  ): Observable<IResponsePaginada<IResponseBodegaFr[]>> {
    let params = new HttpParams();
    if (page != null) params = params.set("page", page);
    if (limit != null) params = params.set("limit", limit);
    return this._httpClient.get<IResponsePaginada<IResponseBodegaFr[]>>(
      URL_BODEGA,
      { params }
    );
  }
  createBodega(
    bodega: ICreateBodegaFrModel
  ): Observable<IResponse<IResponseBodegaFr>> {
    return this._httpClient.post<IResponse<IResponseBodegaFr>>(
      URL_BODEGA,
      bodega
    );
  }
