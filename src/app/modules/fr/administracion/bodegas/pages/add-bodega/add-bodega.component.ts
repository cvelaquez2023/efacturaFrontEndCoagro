...
export class AddBodegaComponent {
	...
	private _loadBodegasErpDisponibles(): void {
		this._bodegaFrApiService.getBodegas().subscribe({
			next: (response) => {
				if (response.success) {
					this.bodegasErpDisponibles = response.result.filter((b) => !!b.BODEGA_ERP && b.BODEGA_ERP === b.BODEGA);
					if (this.bodegasErpDisponibles.length === 0) {
						this._snotifyService.warning('Primero carga una bodega del ERP desde "Cargar Bodega"', {
	...
	private _edit(): void {
		this._bodegaFrApiService.updateBodega(this.ediData.BODEGA, { nombre: this.nombreField.value as string }).subscribe({
			next: (response) => {
				if (response.success) {
					this.bodegaForm.reset();
