...
export class RazonVisitaApiService {
	...
	getRazonVisita(): Observable<IResponse<IResponseRazonVisita[]>> {
		return this._httpClient.get<IResponse<IResponseRazonVisita[]>>(URL_RAZON_VISITA);
	}
	createRazonVisita(razonVisita: ICreateRazonVisitaModel): Observable<IResponse<number>> {
		return this._httpClient.post<IResponse<number>>(URL_RAZON_VISITA, razonVisita);
	}
	...
	updateRazonVisita(efectVisita: string, data: ICreateRazonVisitaModel): Observable<IResponse<number>> {
		const url = URL_RAZON_VISITA + '/' + encodeURIComponent(efectVisita);
