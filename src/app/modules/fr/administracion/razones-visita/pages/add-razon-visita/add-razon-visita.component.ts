...
export class AddRazonVisitaComponent {
  ...
  private _loadFormGroup(): void {
    ...
    if (this.ediData) {
      ...
      this.razonVisitaForm.controls["efectVisita"].setValue(
        this.ediData.EFECT_VISITA
      );
      this.razonVisitaForm.controls["efectVisita"].disable();
      this.razonVisitaForm.controls["descripcion"].setValue(
  ...
  private _edit(razonVisita: ICreateRazonVisitaModel): void {
    this._razonVisitaApiService
      .updateRazonVisita(this.ediData.EFECT_VISITA, razonVisita)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.razonVisitaForm.reset();
