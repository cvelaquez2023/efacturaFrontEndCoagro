...
export class VisitasPageComponent implements OnInit, AfterViewInit {
	...
	displayedColumns: string[] = ['RUTA', 'CLIENTE', 'INICIO', 'RAZON', 'TIPO', 'actions'];
	@ViewChild(MatSort)
	sort!: MatSort;
			...
			buttons: [
				{
					...
					action: (toast) => {
						...
						this._visitaApiService.deleteVisita(element.RUTA, element.CLIENTE, element.INICIO).subscribe((response) => {
							if (response.success) {
