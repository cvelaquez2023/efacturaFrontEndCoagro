...
export class ArticulosPageComponent implements OnInit, AfterViewInit {
	...
	displayedColumns: string[] = ['codigo', 'descripcion', 'bodega', 'grupoArticulo', 'actions'];
	...
	private _loadArticulos(): void {
		forkJoin({
			...
			const descripcionPorGrupo = new Map((grupos.success ? grupos.result : []).map((g) => [g.GRUPO_ARTICULO, g.DESCRIPCION] as [string, string]));
			this.grupoPorArticulo = new Map(
				asociaciones.map((a) => [a.ARTICULO, `${a.GRUPO_ARTICULO} - ${descripcionPorGrupo.get(a.GRUPO_ARTICULO) ?? ''}`] as [string, string])
			);
	...
	private _cargarTodosLosArticulos(): Observable<IResponseArticuloFr[]> {
		return this._articuloFrApiService.getArticulos(1, 100).pipe(
			...
			reduce((acumulado: IResponseArticuloFr[], response) => (response.success ? acumulado.concat(response.result) : acumulado), [] as IResponseArticuloFr[])
		);
	}
