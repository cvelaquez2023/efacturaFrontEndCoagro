...
export class ArticuloFrApiService {
	...
	getArticulos(page?: number, limit?: number): Observable<IResponsePaginada<IResponseArticuloFr[]>> {
		let params = new HttpParams();
		if (page != null) params = params.set('page', page);
		if (limit != null) params = params.set('limit', limit);
		return this._httpClient.get<IResponsePaginada<IResponseArticuloFr[]>>(URL_ARTICULO, { params });
	}
	createArticulo(articulo: ICreateArticuloFrModel): Observable<IResponse<IResponseArticuloFr>> {
		return this._httpClient.post<IResponse<IResponseArticuloFr>>(URL_ARTICULO, articulo);
	}
	updateArticulo(articulo: string, data: ICreateArticuloFrModel): Observable<IResponse<number>> {
