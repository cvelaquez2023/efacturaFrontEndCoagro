...
export class AgentesPageComponent implements OnInit, OnDestroy {
	...
	displayedColumns: string[] = ['codigo', 'nombre', 'empleado', 'telefono', 'correo', 'comision'];
	...
	private _cargarPagina(): void {
		this.cargando = true;
		this._vendedorErpApiService.getVendedoresErp(this.q, this.page, this.limit).subscribe({
			next: (response) => {
				this.cargando = false;
				if (response.success) {
					...
					this._snotifyService.error(response.errors?.[0] ?? 'Error al consultar los vendedores del ERP', {
			...
			error: () => {
				this.cargando = false;
				this._snotifyService.error('No fue posible consultar los vendedores del ERP', { position: SnotifyPosition.rightTop });
			}
		});
	}
}
