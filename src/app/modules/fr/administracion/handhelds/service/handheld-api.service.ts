...
import {
  ICreateHandheldModel,
  IResponseHandheld,
} from "../model/handheld-fr-model-interface";
...
export class HandheldApiService {
  ...
  getHandhelds(estado?: string): Observable<IResponse<IResponseHandheld[]>> {
    const url =
      URL_HANDHELD + (estado ? "?estado=" + encodeURIComponent(estado) : "");
    return this._httpClient.get<IResponse<IResponseHandheld[]>>(url);
  }
  createHandheld(
    handheld: ICreateHandheldModel
  ): Observable<IResponse<IResponseHandheld>> {
    return this._httpClient.post<IResponse<IResponseHandheld>>(
      URL_HANDHELD,
      handheld
    );
  }
