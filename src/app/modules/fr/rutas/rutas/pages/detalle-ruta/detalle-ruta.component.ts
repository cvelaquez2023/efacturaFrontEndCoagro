...
import {
  ICompaniaConsecutivoRuta,
  IRutaCabeceraAsignacion,
} from "../../model/detalle-ruta-model-interface";
import {
...
export class DetalleRutaComponent implements OnInit, AfterViewInit {
    ...
    this._clienteApiService.getClientes().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          const guardados = this._asignacionStore.getClientesRuta(
            this.ruta.RUTA
          );
          const asignadosPorCodigo = new Map(
            guardados.map((c) => [c.cliente, c])
          );
  ...
  ngAfterViewInit(): void {
    ...
    this.filasClientes.filterPredicate = (fila, filtro) =>
      `${fila.cliente} ${fila.nombre}`.toLowerCase().includes(filtro);
  }
  ...
  get companiasTexto(): string {
    return this.filas.length
      ? this.filas.map((f) => f.compania).join(", ")
      : "Sin compañías definidas";
  }
  ...
  abrirPickerNcf(
    fila: ICompaniaConsecutivoRuta,
    campo: "ncfConsumidorFinal" | "ncfCreditoFiscal"
  ): void {
    this._dialog
      .open(F1SelectorComponent, {
        width: "40%",
  ...
  abrirPickerResolucion(
    fila: ICompaniaConsecutivoRuta,
    campo: "resolucionFactura" | "resolucionDevolucion"
  ): void {
    this._dialog
      .open(F1SelectorComponent, {
        width: "40%",
        data: {
          titulo: "Seleccionar Resolución",
          items: RESOLUCIONES_DISPONIBLES,
          columnaCodigo: "Resolución",
          columnaNombre: "Descripción",
        },
      })
      .afterClosed()
  ...
  cambiarDia(fila: IFilaClienteRuta, nuevoDia: DiaSemana | ""): void {
    ...
    const enElMismoDia = this.filasClientes.data.filter(
      (f) => f.dia === nuevoDia && f !== fila
    );
    fila.dia = nuevoDia;
  ...
  private _avisarGuardado(cambioActiva: boolean): void {
    this._snotifyService.info(
      "El detalle se guardó (consecutivos, agente y clientes solo en esta sesión; sin backend aún)",
      {
        position: SnotifyPosition.rightTop,
      }
    );
