...
export class AsignacionRutaMockStoreService {
  ...
  inicializarSimulacion(codigosClientes: string[], rutaActual: string): void {
    ...
    const yaTieneAsignacion =
      this._asignacionesPorRuta.has(rutaActual) &&
      this._asignacionesPorRuta.get(rutaActual)!.length > 0;
    if (!yaTieneAsignacion && codigosClientes.length > 0) {
      const diasSemilla: DiaSemana[] = [
        "Lunes",
        "Martes",
        "Miercoles",
        "Jueves",
      ...
      const semilla: IClienteRuta[] = codigosClientes
        .slice(0, Math.min(3, codigosClientes.length))
        .map((codigo, i) => ({
          cliente: codigo,
          nombre: "",
