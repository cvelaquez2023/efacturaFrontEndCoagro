...
import { ICreateRutaClienteModel, IResponseRutaCliente } from '../model/asignacion-ruta-model-interface';
...
export class RutaClienteApiService {
	...
	getRutaCliente(filtro: IFiltroRutaCliente = {}): Observable<IResponsePaginada<IResponseRutaCliente[]>> {
		...
		return this._httpClient.get<IResponsePaginada<IResponseRutaCliente[]>>(URL_RUTA_CLIENTE, { params });
	}

	createRutaCliente(data: ICreateRutaClienteModel): Observable<IResponse<IResponseRutaCliente>> {
		return this._httpClient.post<IResponse<IResponseRutaCliente>>(URL_RUTA_CLIENTE, data);
	}

	deleteRutaCliente(ruta: string, cliente: string, dia: number): Observable<IResponse<number>> {
		const url = `${URL_RUTA_CLIENTE}/${encodeURIComponent(ruta)}/${encodeURIComponent(cliente)}/${dia}`;
	...
	updateRutaCliente(ruta: string, cliente: string, dia: number, orden: number): Observable<IResponse<number>> {
		const url = `${URL_RUTA_CLIENTE}/${encodeURIComponent(ruta)}/${encodeURIComponent(cliente)}/${dia}`;
	...
	getAllRutaCliente(ruta: string): Observable<IResponseRutaCliente[]> {
		const pagina = (page: number) => this.getRutaCliente({ ruta, page, limit: LIMIT_PAGINA });
		return pagina(1).pipe(
			...
			reduce((acumulado: IResponseRutaCliente[], response) => acumulado.concat(response.result || []), [] as IResponseRutaCliente[])
		);
	}
}
