...
import { ICreateRutaAsignadaModel, IResponseRutaAsignada } from '../model/asignacion-ruta-model-interface';
...
export class RutaAsignadaApiService {
	...
	getRutasAsignadas(): Observable<IResponse<IResponseRutaAsignada[]>> {
		return this._httpClient.get<IResponse<IResponseRutaAsignada[]>>(URL_RUTA_ASIGNADA);
	}
	createRutaAsignada(data: ICreateRutaAsignadaModel): Observable<IResponse<IResponseRutaAsignada>> {
		return this._httpClient.post<IResponse<IResponseRutaAsignada>>(URL_RUTA_ASIGNADA, data);
	}
	updateRutaAsignada(ruta: string, data: Omit<ICreateRutaAsignadaModel, 'ruta'>): Observable<IResponse<number>> {
