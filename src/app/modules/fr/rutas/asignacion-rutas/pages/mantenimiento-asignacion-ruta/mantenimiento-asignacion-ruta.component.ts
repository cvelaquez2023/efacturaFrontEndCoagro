...
import { AGENTES_DISPONIBLES, COMPANIAS_DISPONIBLES } from '../../model/asignacion-ruta-mock-data';
import { DIA_NUMERO, DIAS_SEMANA, DiaSemana, IAsignacionRutaFr, IResponseRutaCliente } from '../../model/asignacion-ruta-model-interface';
...
export class MantenimientoAsignacionRutaComponent implements OnInit, AfterViewInit {
	...
	ngOnInit(): void {
		this._clienteAsocRtApiService.getClientesConNombre().subscribe({
			next: (response) => {
				...
				if (response.success) {
					this.clientes.data = [...response.result].sort((a, b) => a.CODIGO.localeCompare(b.CODIGO));
				} else {
			...
			error: () => {
				this.cargandoClientes = false;
				this._snotifyService.error('No fue posible consultar los clientes de la ruta', { position: SnotifyPosition.rightTop });
			}
		});
	...
	ngAfterViewInit(): void {
		...
		this.clientes.filterPredicate = (c, filtro) => `${c.CODIGO} ${c.NOMBRE}`.toLowerCase().includes(filtro);
	}
	...
	dobleClickCliente(cliente: IResponseClienteConAsoc): void {
		if (!this.modelo.ruta) {
			this._snotifyService.warning('Seleccione primero una Ruta', { position: SnotifyPosition.rightTop });
			return;
		}
		if (!this.diaSeleccionado) {
			this._snotifyService.warning('Seleccione primero un día en el calendario', { position: SnotifyPosition.rightTop });
			return;
		}
		if (this._asignando) return;

		const dia = this.diaSeleccionado;
		const asignacionActual = this.asignaciones.find((a) => a.CLIENTE === cliente.CODIGO);
		if (asignacionActual && asignacionActual.DIA === DIA_NUMERO[dia]) {
			this._snotifyService.warning(`${cliente.CODIGO} ya está asignado al día ${dia}`, { position: SnotifyPosition.rightTop });
			return;
		}
		...
		const eliminarAnterior$: Observable<IResultadoAsignacion> = asignacionActual
			? this._rutaClienteApiService
					.deleteRutaCliente(this.modelo.ruta, cliente.CODIGO, asignacionActual.DIA)
		...
		eliminarAnterior$
			.pipe(
				switchMap((eliminarResponse) => {
					...
					const enEseDia = this.asignaciones.filter((a) => a.DIA === DIA_NUMERO[dia] && a.CLIENTE !== cliente.CODIGO);
					return this._rutaClienteApiService
						.createRutaCliente({ ruta: this.modelo.ruta, cliente: cliente.CODIGO, dia: DIA_NUMERO[dia], orden: enEseDia.length + 1 })
						.pipe(map((r) => ({ success: r.success, errors: r.errors })));
				})
			)
			.subscribe({
				next: (response) => {
					...
					this._snotifyService.info(`${cliente.CODIGO} se asignó al día ${dia}`, { position: SnotifyPosition.rightTop });
					this._cargarAsignaciones();
				},
				error: () => {
					this._asignando = false;
					this._snotifyService.error('No fue posible asignar el cliente al día', { position: SnotifyPosition.rightTop });
				}
			});
	}
	...
	clientesDelDia(dia: DiaSemana): IResponseRutaCliente[] {
		return this.asignaciones.filter((a) => a.DIA === DIA_NUMERO[dia]).sort((a, b) => a.ORDEN - b.ORDEN);
	}
	...
	quitarAsignacion(asignacion: IResponseRutaCliente): void {
		if (this._asignando) return;
		this._asignando = true;
		this._rutaClienteApiService.deleteRutaCliente(asignacion.RUTA, asignacion.CLIENTE, asignacion.DIA).subscribe({
			next: (response) => {
				this._asignando = false;
				if (!response.success) {
					this._snotifyService.error(response.errors[0], { position: SnotifyPosition.rightTop });
				...
				this._snotifyService.info(`${asignacion.CLIENTE} se quitó de ese día`, { position: SnotifyPosition.rightTop });
				this._cargarAsignaciones();
			},
			error: () => {
				this._asignando = false;
				this._snotifyService.error('No fue posible quitar la asignación', { position: SnotifyPosition.rightTop });
			}
		});
	}
	...
	moverOrden(dia: DiaSemana, asignacion: IResponseRutaCliente, direccion: 'arriba' | 'abajo'): void {
		if (this._asignando) return;

		const clientesDia = this.clientesDelDia(dia);
		const index = clientesDia.findIndex((a) => a.CLIENTE === asignacion.CLIENTE);
		const indexVecino = direccion === 'arriba' ? index - 1 : index + 1;
		if (index === -1 || indexVecino < 0 || indexVecino >= clientesDia.length) return;

		const vecino = clientesDia[indexVecino];
		this._asignando = true;
		forkJoin([
			this._rutaClienteApiService.updateRutaCliente(asignacion.RUTA, asignacion.CLIENTE, asignacion.DIA, vecino.ORDEN),
			this._rutaClienteApiService.updateRutaCliente(vecino.RUTA, vecino.CLIENTE, vecino.DIA, asignacion.ORDEN)
		]).subscribe({
			next: ([r1, r2]) => {
				this._asignando = false;
				if (!r1.success || !r2.success) {
					this._snotifyService.error(r1.errors[0] ?? r2.errors[0], { position: SnotifyPosition.rightTop });
				}
				this._cargarAsignaciones();
			},
			error: () => {
				this._asignando = false;
				this._snotifyService.error('No fue posible cambiar el orden', { position: SnotifyPosition.rightTop });
				this._cargarAsignaciones();
			}
		});
	}
	...
	private _cargarAsignaciones(): void {
		...
		this._rutaClienteApiService.getAllRutaCliente(this.modelo.ruta).subscribe({
			...
			error: () => {
				this.cargandoAsignaciones = false;
				this._snotifyService.error('No fue posible consultar los días asignados de la ruta', { position: SnotifyPosition.rightTop });
			}
		});
	}
	...
	abrirPickerRuta(): void {
		...
		this._rutaApiService.getRutas().subscribe({
			next: (response) => {
				...
				const items: IF1Item[] = response.result.map((r) => ({ codigo: r.RUTA, nombre: r.DESCRIPCION }));
	...
	abrirPickerCompania(): void {
		this._abrirF1('Seleccionar Compañía', COMPANIAS_DISPONIBLES, (seleccion) => {
	...
	abrirPickerBodega(): void {
		this._bodegaAsocRtApiService.getBodegasConNombre().subscribe({
			next: (response) => {
				...
				const items: IF1Item[] = response.result.map((b) => ({ codigo: b.CODIGO, nombre: b.NOMBRE }));
	...
	guardar(): void {
		if (!this.modelo.ruta) {
			this._snotifyService.warning('Debe seleccionar una ruta', { position: SnotifyPosition.rightTop });
			return;
		}
