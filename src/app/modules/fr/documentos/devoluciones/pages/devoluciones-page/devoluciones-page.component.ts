...
const MOCK_DEVOLUCIONES: IDevolucionFr[] = [
	{ codigo: 'DEV0001', compania: 'COAGRO2', ruta: 'R001', cliente: 'CL001', bodega: 'BODV', documentoReferencia: 'FAC0045', estado: 'Nuevo', anulada: false, cantidadLineas: 3, fecha: '2026-08-05', lote: '' },
	{ codigo: 'DEV0002', compania: 'COAGRO2', ruta: 'R002', cliente: 'CL003', bodega: 'BODV', documentoReferencia: 'FAC0051', estado: 'Nuevo', anulada: false, cantidadLineas: 1, fecha: '2026-08-06', lote: '' },
	{ codigo: 'DEV0003', compania: 'COAGRO2', ruta: 'R001', cliente: 'CL002', bodega: 'BODV', documentoReferencia: '', estado: 'Nuevo', anulada: false, cantidadLineas: 2, fecha: '2026-08-07', lote: '' }
];
...
export class DevolucionesPageComponent implements OnInit, AfterViewInit {
	...
	displayedColumns: string[] = ['codigo', 'ruta', 'cliente', 'documentoReferencia', 'cantidadLineas', 'fecha', 'lote', 'actions'];
	...
	clickEdit(element: IDevolucionFr): void {
		this._dialog
			...
			.subscribe((result: IDevolucionFr) => {
				if (result) {
					this._data = this._data.map((item) => (item.codigo === element.codigo ? result : item));
					this.listDevoluciones.data = this._data;
					this._snotifyService.info('El lote se registro sin problema', { position: SnotifyPosition.rightTop });
				}
			});
	}
}
