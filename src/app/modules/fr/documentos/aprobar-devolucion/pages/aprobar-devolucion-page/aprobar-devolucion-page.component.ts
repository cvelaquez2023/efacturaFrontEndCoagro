...
export class AprobarDevolucionPageComponent implements OnInit, AfterViewInit {
  ...
  listDevoluciones = new MatTableDataSource<
    IDevolucionFr & { seleccionada: boolean }
  >();
  ...
  verDetalle(element: IDevolucionFr): void {
    this._dialog.open(DetalleAprobarDevolucionComponent, {
      width: "45%",
      data: element,
    });
  }
  ...
  aprobarSeleccionadas(): void {
    const seleccionadas = this._data.filter(
      (item) => item.seleccionada && item.estado === "Nuevo"
    );
    if (!seleccionadas.length) {
      this._snotifyService.warning(
        "Debe seleccionar al menos una devolucion en estado Nuevo",
    ...
    this._data = this._data.map((item) =>
      item.seleccionada
        ? { ...item, estado: "Aprobado" as const, seleccionada: false }
        : item
    );
    this.listDevoluciones.data = this._data;
    this._snotifyService.info(
      "Las devoluciones seleccionadas fueron aprobadas",
      { position: SnotifyPosition.rightTop }
    );
  }
}
