...
import { IResponseDtes, ITicketPdf } from '@app/modules/ci/articulo/model/articulo-api-model-interface';
...
export class ListaClienteDteComponent implements OnInit, AfterViewInit {
	...
	*/
	...
	clickTicket(element: any): void {
		...
		this._dteApiService.posticketPdf(element.Dte_Id).subscribe({
			next: (response) => {
				...
				TicketPDF(empresa, receptor, _produc, [resumenFixed], dte, iva, cliente, vendedor, documento);
			}
		});
		/*
		const products = [
	...
	clickCH(element: any): void {
		this._dteApiService.postDteMh(element.Dte, this.datoUsuario.empresa).subscribe({
			next: (response) => {
				if (response.success) {
