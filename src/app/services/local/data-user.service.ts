...
export class DataUserService {
  ...
  getToken(): string | undefined {
    const tokenUser = this._sessionStorageService.getItem<IDataUser>(
      KEYS_WEB_STORAGE.DATA_USER
    );
    return tokenUser?.token;
  }
  getFullName(): string | null {
    const dataUser = this._sessionStorageService.getItem<IDataUser>(
      KEYS_WEB_STORAGE.DATA_USER
    );
    if (dataUser !== null) {
  ...
  existsStorega(): boolean {
    return (
      this._sessionStorageService.getItem<IDataUser>(
        KEYS_WEB_STORAGE.DATA_USER
      ) != null
    );
  }
